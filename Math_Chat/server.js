...
var express 	= require('express'),
	app			= express(),
	appuse = app.use('/varSocketURI.js', function(req, res) {
	    var port = 8000;
...
io.sockets.on('connection', function(socket){
	...
	socket.on('update', function (element) {
		socket.broadcast.to(element.room).emit('anEvent', element.svg);
	});
	
	socket.on('removed', function (element) {
		socket.broadcast.to(element.room).emit('delete', element.id);
	});
	
	socket.on('sendsvg', function (msg) {
		// Send only one time the svg
		if (clientNeedSvgfromRoom[msg.room])	{
			clientNeedSvgfromRoom[msg.room]= false;
			socket.broadcast.to(msg.room).emit('setsvg', msg.svg);
		}
	})
...
function subscribe(socket, data){
	...
	if(rooms.indexOf('/' + data.room) < 0){
		...
		clientNeedSvgfromRoom[data.room] = false;
	}
	...
	// send him the current svg drawing if there's more than one client connected
	if (countClientsInRoom(data.room) > 1)	{
		clientNeedSvgfromRoom[data.room] = true;
		socket.broadcast.to(data.room).emit('getsvg', { room: data.room });
	}
}

...
var lastMouseDown_x = 100;
var lastMouseDown_y = 100;
var lastKeyPress;
var shortcutIndex = 0;
...
var remapElement = this.remapElement = function(selected,changes,m) {
  ...
  var remap = function(x,y) { return transformPoint(x,y,m); },
    ...
    finishUp = function() {
      //if(doSnapping) for(var o in changes) changes[o] = snapToGrid(changes[o]); //**MDP this line causes x-coordinates to snap too
...
//      if (j == 0) selectedBBoxes[0] = svgedit.utilities.getBBox(elem);
  ...
  if (showGrips || selectedElements.length == 1 && selectedElements[0].nodeName != 'text') selectorManager.requestSelector(selectedElements[0]).showGrips(true) //**MDP to prevent text from resizing
  ...
  var mouseDown = function(evt)
  {
    ...
    switch (current_mode) {
      case "select":
        ...
        //**MDP[ : Hack to make sure cursor does not get selected
        var w = svgCanvas.getElem('math_cursor');

        //Not sure why I need the following as well as frame line as well
        document.getElementById('workarea').focus();
        //needed if there is a Chat Frame
        if (window.self !== window.top) {parent.frames[0].focus()};
        if (w != null) {w.setAttribute('style', 'pointer-events:none');};
        //**MDP]
        ...
        if (mouse_target != svgroot) {
          ...
          //**MDP (    -- Move selected elements by bounding box
          var moveMode = false;
          var i = selectedElements.length;
          for (i=0; i < selectedElements.length; i++) {
            var selected = selectedElements[i];
            if (selected != null) {
              var sBB = svgedit.utilities.getBBox(selected);
                if (x > sBB.x && y > sBB.y && x < sBB.x + sBB.width && y < sBB.y +sBB.height) moveMode=true;
            }
          }
    ...
    switch (current_mode)
    {
      case "select":
        ...
        if (selectedElements[0] !== null) {
          ...
          if(curConfig.gridSnapping){
          //  dx = snapToGrid(dx);
            dy = snapToGrid(dy); //**MDP SnapToGrid.. only y
          }
        ...
        if (selectedElements[0] != null) {
          ...
          if (selectedElements.length == 1) {
            ...
            if (selected.tagName == "text") {
              ...
              selectorManager.requestSelector(selected).showGrips(false); //**MDP
              if (selected.textContent == "\u222b") selectorManager.requestSelector(selected).showGrips(true); //**MDP RESIZEABLE
      ...
      setTimeout(function() {
        ...
        if(current_mode === "path") {
          ...
          if(curConfig.selectNew) {
            clearSelection(); //**MDP after line or sketch... get rid of bounding box
          //  selectOnly([element], true); //**MDP
          }
        }
...
this.setTextContent = function(val) {
  ...
  //**MDP[  This is to send the character as soon as it is tpyed
...
//Removes an element from the DOM by his id and add the
//change to the history stack
this.deleteElement = function(elementId) {
     var batchCmd = new BatchCommand("Delete Elements");
     var selected = svgcontent.getElementById(elementId);
//     if (selected == null) return;
...
// 			// Use grid snap value regardless of zoom level
	...
	// 		"<h1> <font color=red> s  </font>  S &#x03c3  &#x2211 &#x222b &#x221a </h1>";
	// 			newText.textContent = 's';
	// 	};
  //
	// 	if (shortcutIndex==1) {
	// 		document.getElementById('floatingContent').innerHTML =
	// 		"<h1> s <font color=red> S  </font>  &#x03c3  &#x2211 &#x222b &#x221a </h1>";
	// 		newText.textContent = 'S';
  //
	// 	};
  //
	// 	if (shortcutIndex==2) {
	// 		document.getElementById('floatingContent').innerHTML =
	// 		"<h1> s S <font color=red> &#x03c3 </font>  &#x2211 &#x222b &#x221a </h1>";
	// 		newText.textContent = '\u03c3';
	// 	};
  //
  //
	// 	if (shortcutIndex==3) {
	// 		document.getElementById('floatingContent').innerHTML =
	// 		"<h1> s S &#x03c3 <font color=red> &#x2211  </font>  &#x222b &#x221a </h1>";
	// 		newText.textContent = '\u2211';
	// 	};
  //
	// 	if (shortcutIndex==4) {
	// 		document.getElementById('floatingContent').innerHTML =
	// 		"<h1> s S &#x03c3 &#x2211 <font color=red> &#x222b  </font> &#x221a </h1>";
	// 		newText.textContent = '\u222b';
	// 	};
  //
	// 	if (shortcutIndex==5) {
	// 		document.getElementById('floatingContent').innerHTML =
	// 		"<h1> s S &#x03c3 &#x2211 &#x222b <font color=red>  &#x221a  </font>  </h1>";
	// 		newText.textContent = '\u221a';
	// 		shortcutIndex=-1;
	// 	};
  //
  ...
  //
	...
	var startBlinking = function () {
		blinking=setInterval(function() {
			svgCanvas.getElem('math_cursor').setAttribute('style', 'pointer-events:none');
			var cursor_state = svgCanvas.getElem('math_cursor').getAttribute('opacity');
			if (cursor_state==1.0) {
				svgCanvas.getElem('math_cursor').setAttribute('opacity', 0.0);
			}
			else {
					svgCanvas.getElem('math_cursor').setAttribute('opacity', 1.0);
			}
			blinking = true;
	}, 600);
	};

...
var emptySvg = '<svg width="640" height="480" xmlns="http://www.w3.org/2000/svg">'
...
///		currentRoom = null,
	...
	function bindSocketEvents(){
		...
		socket.on('roomclients', function(data){
			...
			// Request svg if clients are already in room, create new else
			if (data.clients.length > 0)	{
				needSvg = true;
			}	else	{
				parent.frames[0].setSVG(emptySvg);
			}
		...
		socket.on('anEvent', function(svgelement) {
			...
			// * curStyles - Boolean indicating that current style attributes should be applied first
				parent.frames[0].methodDraw.canvas.addSvgElementFromJson(svgelement); //**MDP

				//**MDP Hack to update selection -- TODO: Fix selection to work with rotation 
				parent.frames[0].methodDraw.canvas.setZoom(1.0); 

		});
		
		socket.on('delete', function(id) 	{
			//alert(id);
			parent.frames[0].methodDraw.canvas.deleteElement(id);
		});
		
		socket.on('getsvg', function(msg)	{
			svgdata = parent.frames[0].getSVG();
			socket.emit('sendsvg', { room: currentRoom, svg:	svgdata});
		});
		
		socket.on('setsvg', function(data) 	{
			// check if needed
			if (needSvg)	{
				parent.frames[0].setSVG(data);
				needSvg = false;
			}
		});
	}
